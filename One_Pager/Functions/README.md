...
| 2 | `GetValuationChartFromExcel` | `GetValuationChartFromExcel` | `void GetValuationChartFromExcel(int recId)` | Fetch valuation-chart source cells from a Zoho Sheet workbook (worksheet `AIAE`) — currently a data-fetch/test stub. |
...
2. Calls the Zoho Sheet API (`https://sheet.zoho.in/api/v2/<resourceId>`, connection `zoho_oauth_connection`) with `method=find`, `scope=workbook`, `is_exact_match=true` to locate the Bloomberg code inside a hardcoded workbook (`resourceId = "ln7pj5c000832533d414d8fd741cc8a945b1a"`).
...
If no matching Banking/Non-Banking sheet is found, it logs a message and makes no update. A `zoho.creator.updateRecord` call is present but commented out (the direct field assignment is used instead).
...
2. Calls the Zoho Sheet API against a hardcoded workbook (`resourceId = "hkx0j4559d438ff334ec68150f377253d3140"`, connection `zoho_oauth_connection`) and logs the response.

This function is currently a **data-fetch / test stub**: the `recId`/record lookup at the top is commented out, the response is only `info`-logged (with a `setFileName("testfile.xlsx")` call), and the `sendmail` block that would attach the result is commented out. It does not write anything back to any record. The valuation-chart data that `generateChart` actually plots is passed in via that function's `iMap` input rather than produced here.
...
1. Reads its input from `basicIO.getParameter('iMap')` (JSON), expecting: `data` (array of `{Date, PE}` rows), pre-computed `plus1` / `minus1` / `mean` (the +1 STD / -1 STD / mean bands), `title` (company name), `token` (an OAuth access token), and `id` (the Creator record id). A commented-out fallback demo block exists for manual runs.
...
The uploaded image is what `FI_AB_One_Pager_Static_HTML` later references via its constructed image-download URL. Errors are caught and written back via `basicIO.write("ERROR: ...")`. Despite the `httpsUploadPng` name, it uploads SVG content.
...
2. **Valuation Chart:** builds an image-download URL pointing at the `Valuation_Chart_Image` field on `One_Page_Module_Report` (using `zoho.appuri` and the field's stored filename via `getSuffix("image/").getPrefix(" lowqual")`). It locates the rendered `<div class="print-header">Valuation Chart</div>…<div class="img-container">` anchor block and replaces its `<img>` with a new `<img src="<new_img_url>">`. If the anchor isn't found, it leaves the HTML unchanged and logs it. A leading code comment warns that the hardcoded token embedded in the image URL looks like a report-perma token rather than an image-download privacy token, and is the likely culprit if the image returns 401/403.
...
Several `sendmail` debug blocks and `info` dumps are commented out.
...
5. Resolves the WorkDrive folder: uses `oneRec.Workdrive_Folder_ID` if set; otherwise creates a subfolder under the hardcoded parent (`hkx0jd92cea3d216e4b5ca9bca83621a6978d`) via `zoho.workdrive.createFolder(..., "zoho_workdrive_connection")` and stores the new folder id back on the record.
...
An earlier approach that fetched the PDF from a `page-perma`/`pdf` print URL via `invokeurl`, plus an extra test `sendmail`, are commented out.
...
4. Builds `output_settings` — combined file name `OnePagers-<monthLabel>-Batch-<batchNumber>.pdf`, hardcoded destination `folder_id = "kftie8153e640d77f419b9d16e6ba4d8a9ad9"`, `overwrite_existing_file = false` — and adds `output_settings` and `input_options` as JSON string parts.
...
Note: this function converts `Print_Layout_HTML` directly and does **not** call `FI_AB_One_Pager_Static_HTML` first, so it assumes the layout HTML was already assembled for each record.
...
OAuth helper. Makes a `POST` to `https://accounts.zoho.in/oauth/v2/token` with `grant_type=refresh_token` (client id, client secret, and refresh token embedded in the query string — redacted with `XXXX`/`xxxx` placeholders in the committed source), reads `access_token` from the response, logs the full response, and returns the token. Used to authorize API calls (e.g. the token passed into `generateChart` via `iMap`).
...
Same as #7 but with a distinct (WorkDrive-scoped) set of OAuth credentials. `POST`s to `https://accounts.zoho.in/oauth/v2/token` with `grant_type=refresh_token`, extracts and returns `access_token`, and logs the response. Provides a WorkDrive-authorized access token for WorkDrive operations. Credentials in source are redacted placeholders.
