void send_meeting_Notes_Second_reminder()
{
	...
	for each  att in attendance_records
	{
		...
		// Check notes
		notes_added = false;
		notes_records = Morning_Meeting_Notes[Meeting_Detail == meetings.ID && Analyst.ID == analyst.ID];
		info notes_records;
		if(notes_records.count() > 0)
		{
			for each  note in notes_records
			{
				if(note.Have_you_attended_today_s_meeting != "")
				{
					attendance_not_marked = false;
					attendance_marked = true;
				}
				else
				{
					attendance_not_marked = true;
		...
		// 		info analyst.Full_Name;
		// 		notes_records.Reminder_5=true;
	}
}
