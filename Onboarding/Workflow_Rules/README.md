...
- **Anomalies:** The Digio **Basic-auth token is hard-coded in plaintext** in the invokeurl header (not redacted in this file) — it should be moved to a Zoho connection/secret. No handling of API failure/non-200 responses, timeouts, or a `verified == false` case beyond an `info` log; on failure the record is silently left unverified.
